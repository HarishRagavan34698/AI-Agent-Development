...
export type Activity = { id: number; agent: string; message: string; createdAt: string }

const memoryActivities: Activity[] = []
...
export async function addActivity(agent: string, message: string) {
  if (!pool) {
...
export async function listActivities() {
  if (!pool) return memoryActivities.slice(0, 20)
